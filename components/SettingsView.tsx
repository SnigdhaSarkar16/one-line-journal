  ...
  import { Download, Mail, User, LogOut, Globe, Send, AlertCircle, HelpCircle, Check } from 'lucide-react';
  ...
  const SettingsView: React.FC<SettingsViewProps> = ({ settings, entries, onUpdate, onLogout }) => {
    ...
    const triggerTestEmail = async () => {
      ...
      try {
        // We pass the email and name so the Edge Function knows where to send the test
        ...
        if (error) {
          if (error.message?.includes('404') || error.message?.includes('Failed to send')) {
            throw new Error("Function 'send-reminders' not found. Please deploy the function code via Supabase CLI.");
          }
          throw error;
        }
          ...
          <section className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl border border-stone-100 shadow-sm space-y-6">
              ...
              <div className="space-y-3 pt-2">
                ...
                {testResult && (
                  <div className={`flex flex-col gap-3 p-4 rounded-xl text-[11px] leading-relaxed animate-in fade-in slide-in-from-top-1 ${
                    testResult.isError ? 'bg-red-50 text-red-700 border border-red-100' : 'bg-green-50 text-green-700 border border-green-100'
                  }`}>
                    <div className="flex items-start gap-2">
                      {testResult.isError ? <AlertCircle size={14} className="mt-0.5 shrink-0" /> : <Check size={14} className="mt-0.5 shrink-0" />}
                      <p className="font-medium">{testResult.message}</p>
                    </div>
                    
                    {testResult.isError && (
                      <div className="bg-white/50 p-3 rounded-lg border border-red-100 mt-1">
                        <div className="flex items-center gap-1.5 mb-1 text-red-800 font-bold uppercase tracking-wider text-[9px]">
                          <HelpCircle size={10} />
                          How to fix
                        </div>
                        <p className="text-red-600/80">
                          1. Ensure RESEND_API_KEY is set in Supabase Secrets.<br/>
                          2. Run <code className="bg-red-100 px-1 rounded">supabase functions deploy send-reminders</code>.<br/>
